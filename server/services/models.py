...
class Availability(models.Model):
    ...
    def clean(self):
        ...
        if overlapping_slots.exists():
            raise ValidationError("This time slot overlaps with an existing availability slot for the same day.")
...
class ServiceRequest(models.Model):
    ...
    def clean(self):
        ...
        if 'subject' not in self.request_msg or 'body' not in self.request_msg:
            raise ValidationError("Request message must contain 'subject' and 'body' fields")
...
def get_service_popular_plans(service):
    ...
    plans_data = ServiceRequest.objects.filter(
        service=service
    ).values(
        'plan'
    ).annotate(
